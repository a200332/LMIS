...
namespace CITI.EVO.Tools.Security
{
    ...
    public class UmUtil : ISerializable, IDisposable
    {
        ...
        {
            var loginPageUrl = MakeCorrectUrl(PermissionUtil.LoginPage);
        ...
        {
            var changePasswordPageUrl = MakeCorrectUrl(PermissionUtil.ChangePasswordPage);
        ...
        private String MakeCorrectUrl(String url)
        {
            if (String.IsNullOrWhiteSpace(url))
                return url;

            return Regex.Replace(url, "{currentHost}", Request.Url.Host, RegexOptions.IgnoreCase);
        }
